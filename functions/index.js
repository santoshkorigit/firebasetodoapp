...
function timestampHandler(timestamp) {
	let fullTime;
	let firedate = admin.firestore.Timestamp.fromDate(new Date(timestamp._seconds * 1000));
	...
	if (req.body._method === "DELETE") {
		...
		let deleteDoc = db
			...
			.catch(error => {
				...
				res.redirect("/create?status=failed");
				// res.send("failed");
				res.end();
			});
	} else {
		...
		var taskDetail = taskDetailRef
			...
			.then(() => {
				console.log("success");
				res.redirect("/task/" + task_id + "?update=true");
				res.end();
				return true;
			})
			.catch(error => {
				console.error("failed ", error);
				res.redirect("/task/" + task_id + "?update=false");
				// res.send("failed");
				res.end();
			});
		// res.send("success");
...
app.get("/create", jsonParser, (request, response, next) => {
	var taskData = {
		status: "",
		title: "",
		detail: "",
	...
	response.render("create", taskData);
});

app.post("/create", (req, res) => {
	console.log("INSIDE create $$$$" + req.body);
	let id = Math.floor(new Date() / 1000);
	let dueTimeStamp = dateToFirebaseTimestamp(req.body.due);
	console.log("dueTimeStamp " + JSON.stringify(dueTimeStamp));
	req.body.due = dueTimeStamp;
	db.collection("task")
		.doc("test")
